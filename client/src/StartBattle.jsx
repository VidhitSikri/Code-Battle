...
const StartBattle = () => {
  ...
  // Local index to iterate over questions from battle.questions array
  const [questionIndex, setQuestionIndex] = useState(0);
  // Timer state for quality mode (10 minutes = 600 seconds)
  const [timer, setTimer] = useState(600);
  // Holds the editor instance for language switching
  ...
  // Listen for socket events "newQuestion" and "scoreUpdate"
  useEffect(() => {
    if (!socket) return;
    socket.on("newQuestion", (data) => {
      // When opponent receives a new question generated by creator
      setCurrentQuestion(data.question);
      // If quality mode, reset timer
      if (battle && battle.mode === "quality") {
        setTimer(600);
      }
    });
    socket.on("scoreUpdate", (data) => {
      // Update scores from socket event
  ...
  // Timer for quality-based battle mode
  useEffect(() => {
    if (battle && battle.mode === "quality" && currentQuestion) {
      const interval = setInterval(() => {
        setTimer((prev) => {
          if (prev <= 1) {
            // Time's up: no point awarded, move to next question
            clearInterval(interval);
            setCurrentQuestion(null);
            // Optionally, you can also notify opponent of timeout here.
  ...
  // Simulate opponent typing if a question is shown (for demo)
  useEffect(() => {
    if (currentQuestion) {
  ...
  // Handler for generating a new question (creator only)
  const handleGenerateQuestion = () => {
    if (!isCreator || currentQuestion) return; // only allow if no active question
    if (battle && battle.questions && battle.questions.length > questionIndex) {
      setIsGeneratingQuestion(true);
      // Use question from battle.questions using current questionIndex.
      const newQuestion = {
        ...battle.questions[questionIndex],
        // For testing, constraints remain hardcoded.
        constraints: `
• 2 <= nums.length <= 10^4
• -10^9 <= nums[i] <= 10^9
• -10^9 <= target <= 10^9
• Only one valid answer exists.
      ...
      // Set current question and update questionIndex for next question.
      ...
      // Emit socket event so that opponent receives the question.
  ...
  // Handler for submitting code; for testing, pressing it awards a point.
  const handleSubmitCode = () => {
    // For testing, simply award a point to the current user.
    ...
    setScores(newScores);
    // Emit socket event so that both users update scores.
    ...
    setCurrentQuestion(null);
    // Reset timer for quality mode if applicable.
    if (battle.mode === "quality") {
      setTimer(600);
    }
  };
      ...
      <header className="bg-gray-800/50 border-b border-gray-700/50 backdrop-blur-sm sticky top-0 z-10">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            {battle.mode === "quality" && currentQuestion && (
          ...
          <div className="bg-gray-800/50 border border-gray-700 rounded-xl overflow-hidden">
            {!currentQuestion ? (
              ...
              <div className="p-4">
                ...
                {/* Only the creator sees this "Next Question" button if the current question is done */}
                {isCreator && (
